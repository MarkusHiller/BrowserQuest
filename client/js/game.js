...
//                                            self.player.getWeaponName());
                        ...
                        self.client.onInventoryUpdate(function (slot, itemId) {
                            self.player.inventory.updateSlot(slot, itemId);
                            self.app.updateInventorySlotIcon(slot);
                        });
                ...
                tryUseItem: function (slot) {
                    if (this.player.inventory.isUsableItem(slot)) {
                        this.client.sendUseItem(slot);
                    }
                },
                tryDeleteItem: function (slot) {
                    if (this.player.inventory.hasItem(slot)) {
                ...
                buildItemContext: function (slot) {
                    var self = this;
                    var contextObj = "";
                    var context = this.player.inventory.getItemContext(slot);
                    if (context.length === 0)
                        return contextObj;
                    contextObj = $(document.createElement('ul'));
                    for (var i = 0; i < context.length; i++) {
                        contextObj.append(
                                $(document.createElement('li'))
                                .text(context[i] === "Move" ? "Move to ..." : context[i])
                                .click(slotAction(i))
                                );
                    }

                    function slotAction(x) {
                        return function () {
                            self['try' + context[x] + "Item"](slot);
                            $('#item_context').hide();
                        };
                    }
