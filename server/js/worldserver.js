...
//                }
        ...
        this.onPlayerEnter(function (player) {
            log.info(player.name + " has joined " + player.map);
            
            if (!player.hasEnteredGame) {
                self.incrementPlayerCount();
            }
            
            player.mapServer.pushRelevantEntityListTo(player);

            var move_callback = function (x, y) {
                log.debug(player.name + " is moving to (" + x + ", " + y + ") on map " + player.map + ".");

                player.forEachAttacker(function (mob) {
                    var target = player.mapServer.getEntityById(mob.target);
                    if (target) {
                        var pos = player.mapServer.findPositionNextTo(mob, target);
                        if (mob.distanceToSpawningPoint(pos.x, pos.y) > 50) {
                            ...
                            log.debug("bevorMove");
                            player.mapServer.moveEntity(mob, pos.x, pos.y);
                            log.debug("AfterMove");
                        }
                    }
                });
            ...
            player.onZone(function () { // TODO:: onMapChange
                var hasChangedGroups = player.mapServer.handleEntityGroupMembership(player);

                if (hasChangedGroups) {
                    player.mapServer.pushToPreviousGroups(player, new Messages.Destroy(player));
                    player.mapServer.pushRelevantEntityListTo(player);
                }
            });

            player.onBroadcast(function (message, ignoreSelf) {
                player.mapServer.pushToAdjacentGroups(player.group, message, ignoreSelf ? player.id : null);
            });
            ...
            if (player.mapServer.added_callback) {
                player.mapServer.added_callback();
            }
        });
    ...
    run: function () {
        var self = this;
        var mapNames = ['world_client'];
        _.forEach(mapNames, function (mapName) {
            self.mapServers[mapName] = new MapServer(mapName, self);
        });
        ...
        setInterval(function () {
            ...
            _.forEach(mapNames, function(mapServerName) {
                self.mapServers[mapServerName].processMap();
            });
            //self.processGroups();
            self.processQueues();
...
//            entities = _.keys(player.mapServer.entities);
...
//            var entity = self.mapServers[player.map].getEntityById(id);
//            if (entity) {
...
//    },
    addPlayer: function (player) {
        ...
        player.mapServer = this.mapServers[player.map];
        player.mapServer.addPlayer(player);
    },
    removePlayer: function (player) {
        player.mapServer.broadcast(player.despawn());
...
//    },
    forEachCharacter: function (callback) {
        ...
        //this.forEachMob(callback); //TODO:: called from regen_tick to regen mobs implementing on mapServer?
    },
...
//        log.debug("Nicht hier");
//        var hasChangedGroups = false;
//        if (entity) {
//            var groupId = this.mapServers[entity.map].map.getGroupIdFromPosition(entity.x, entity.y);
