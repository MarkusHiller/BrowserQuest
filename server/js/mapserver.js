...
module.exports = MapServer = cls.Class.extend({
    init: function (mapName, server) {
        var self = this;
        log.debug("init map " + mapName);
    ...
    loadMap: function () {
        ...
        this.map = new Map('./server/maps/' + self.mapName + '.json');
    ...
    pushRelevantEntityListTo: function (player) {
        ...
        if (player) {
            ...
            if (entities) {
                this.server.pushToPlayer(player, new Messages.List(entities));
            }
        }
    },
    pushSpawnsToPlayer: function (player, ids) {
        ...
        _.each(ids, function (id) {
            ...
            if (entity) {
                self.server.pushToPlayer(player, new Messages.Spawn(entity));
            }
        });
...
//                    self.server.pushToPlayer(self.getEntityById(playerId), message);
//                }
//            });
//        } else {
...
//    },
    ...
    broadcastAttacker: function (character) {
        if (character) {
            this.broadcast(character.attack(), character.id);
        }
    ...
    handleHurtEntity: function (entity, attacker, damage) {
        ...
        if (entity.type === 'player') {
            ...
            this.server.pushToPlayer(entity, entity.health());
        }

        if (entity.type === 'mob') {
            // Let the mob's attacker (player) know how much damage was inflicted
            this.server.pushToPlayer(attacker, new Messages.Damage(entity, damage));
        }
        ...
        if (entity.hitPoints <= 0) {
            if (entity.type === "mob") {
                ...
                this.server.pushToPlayer(attacker, new Messages.Kill(mob));
                this.broadcast(mob.despawn()); // Despawn must be enqueued before the item drop
                if (item) {
                    this.broadcast(mob.drop(item));
    ...
    onMobMoveCallback: function (mob) {
        this.broadcast(new Messages.Move(mob));
...
//    },
    ...
    addAsIncoming: function (entity) {
        ...
        if (entity) {
            if (!isItem || isChest || (isItem && !isDroppedItem)) {
                self.incoming.push(entity);
            }
        }
    },
...
//    },
    processMap: function () {
        ...
        if (self.incoming.length > 0) {
            _.forEach(self.incoming, function (entity) {
                if (entity instanceof Player) {
                    self.broadcast(new Messages.Spawn(entity), entity.id);
                } else {
        ...
        if (item) {
            item.handleDespawn({
                ...
                blinkCallback: function () {
                    self.broadcast(new Messages.Blink(item));
                },
                blinkingDuration: 4000,
                despawnCallback: function () {
                    self.broadcast(new Messages.Destroy(item));
